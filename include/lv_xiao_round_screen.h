...
static Arduino_DataBus *bus = new Arduino_ESP32SPI(XIAO_DC, XIAO_CS, SCK, MOSI, MISO, FSPI);
#elif defined(ARDUINO_Seeed_XIAO_nRF52840_Sense) || defined(ARDUINO_Seeed_XIAO_nRF52840)
static Arduino_DataBus *bus = new Arduino_NRFXSPI(XIAO_DC, XIAO_CS, SCK, MOSI, MISO);
#elif defined(ARDUINO_SEEED_XIAO_NRF52840_SENSE) || defined(ARDUINO_SEEED_XIAO_NRF52840)
static Arduino_DataBus *bus = new Arduino_mbedSPI(XIAO_DC, XIAO_CS);
#elif defined(ARDUINO_SEEED_XIAO_RP2040)
static Arduino_DataBus *bus = new Arduino_RPiPicoSPI(XIAO_DC, XIAO_CS, SCK, MOSI, MISO, spi0);
#else
static Arduino_DataBus *bus = new Arduino_HWSPI(XIAO_DC, XIAO_CS);
#endif
static Arduino_GFX *gfx = new Arduino_GC9A01(bus, -1, screen_rotation, true);
#else
#error "Please define a graphics library for display."
#endif

#if LVGL_VERSION_MAJOR == 9
static void xiao_disp_flush( lv_display_t *disp, const lv_area_t *area, uint8_t *px_map )
#elif LVGL_VERSION_MAJOR == 8
static void xiao_disp_flush( lv_disp_drv_t *disp, const lv_area_t *area, lv_color_t *color_p )
#else
...
static void xiao_disp_init(void)
{
#if XIAO_BL > 0
...
static void lv_xiao_disp_init(void)
{
    xiao_disp_init();
...
#elif LVGL_VERSION_MAJOR == 8
static void chsc6x_read( lv_indev_drv_t * indev_driver, lv_indev_data_t * data )
#endif
{
